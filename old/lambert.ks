...
function Lambert {
  ...
  local T00 is arccos(lambda) + lambda * sqrt(1 - lambda^2).
  local T0 is T00 + max_revolutions * constant:pi.
  local T1 is 2 / 3 * (1 - lambda^3).

  set max_revolutions is min(MaxRevolutions(T, lambda), max_revolutions).

  local x_list is InitialGuess().
...
function InitialGuess {
  ...
  local x_list is List().

	if (T>=T00) {
		x_list:add(-(T - T00) / (T - T00 + 4));
	} else if (T<=T1) {
		x_list:add(T1 * (T1 - T) / (0.4 * (1 - lambda^5) * T) + 1);
	} else {
    x_list:add((T / T00)^(0.69314718055994529 / log(T1/T00)) - 1).
	}

  return x_list.
}

function MaxRevolutions {
  parameter
    T,
    lambda.
...
function TimeOfFlight {
  ...
  if dist < lagrange and dist > battin {
    return LagrangeTimeOfFlight(x, revolutions).
  }
...
function LancasterTimeOfFlight {
  ...
  if E < 0 {
    set d to num_revolutions * constant:pi + acos(g).
  } else {
    set d to log(y * (z - lambda * x) + g).
  }
