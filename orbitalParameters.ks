...
function ArgumentOfPeriapsis {
  ...
  return VDot(n_vec, e_vec) / (n_vec:mag * e_vec:mag).
}
...
function FlightPathAngle {
  ...
  return VectorAngle(r_vec, v_vec) - 90.
}
...
function CircularOrbitVelocity {
  ...
  return sqrt(mu / target_radius).
}

// Velocity of the current ship at apoapsis s
function VelocityAtApoapsisShip {
  local apoapsis is ship:obt:apoapsis + ship:body:radius.
  return VelocityAtRadius(apoapsis, AngularMomentumShip():mag).
}
